...
func (lq *ListenerQuery) QueryBeacon() *BeaconQuery {
	query := (&BeaconClient{config: lq.config}).Query()
...
func (lq *ListenerQuery) Clone() *ListenerQuery {
	...
	return &ListenerQuery{
		...
		withBeacon: lq.withBeacon.Clone(),
...
func (lq *ListenerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Listener, error) {
	...
	if query := lq.withBeacon; query != nil {
		if err := lq.loadBeacon(ctx, query, nodes,
			func(n *Listener) { n.Edges.Beacon = []*Beacon{} },
			func(n *Listener, e *Beacon) { n.Edges.Beacon = append(n.Edges.Beacon, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (lq *ListenerQuery) loadBeacon(ctx context.Context, query *BeaconQuery, nodes []*Listener, init func(*Listener), assign func(*Listener, *Beacon)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(beacon.FieldListenerID)
	}
	query.Where(predicate.Beacon(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(listener.BeaconColumn), fks...))
	}))
