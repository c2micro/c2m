...
func (gq *GroupQuery) QueryBeacon() *BeaconQuery {
	query := (&BeaconClient{config: gq.config}).Query()
...
func (gq *GroupQuery) QueryOperator() *OperatorQuery {
	query := (&OperatorClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) QueryMessage() *MessageQuery {
	query := (&MessageClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withBeacon:   gq.withBeacon.Clone(),
		withOperator: gq.withOperator.Clone(),
		withMessage:  gq.withMessage.Clone(),
		withTask:     gq.withTask.Clone(),
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withBeacon; query != nil {
		if err := gq.loadBeacon(ctx, query, nodes, nil,
			func(n *Group, e *Beacon) { n.Edges.Beacon = e }); err != nil {
			return nil, err
		}
	}
	if query := gq.withOperator; query != nil {
		if err := gq.loadOperator(ctx, query, nodes, nil,
			func(n *Group, e *Operator) { n.Edges.Operator = e }); err != nil {
			return nil, err
		}
	}
	if query := gq.withMessage; query != nil {
		if err := gq.loadMessage(ctx, query, nodes,
			func(n *Group) { n.Edges.Message = []*Message{} },
			func(n *Group, e *Message) { n.Edges.Message = append(n.Edges.Message, e) }); err != nil {
			return nil, err
		}
	}
	if query := gq.withTask; query != nil {
		if err := gq.loadTask(ctx, query, nodes,
			func(n *Group) { n.Edges.Task = []*Task{} },
			func(n *Group, e *Task) { n.Edges.Task = append(n.Edges.Task, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GroupQuery) loadBeacon(ctx context.Context, query *BeaconQuery, nodes []*Group, init func(*Group), assign func(*Group, *Beacon)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "bid" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (gq *GroupQuery) loadOperator(ctx context.Context, query *OperatorQuery, nodes []*Group, init func(*Group), assign func(*Group, *Operator)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Group)
	for i := range nodes {
		fk := nodes[i].Author
...
func (gq *GroupQuery) loadMessage(ctx context.Context, query *MessageQuery, nodes []*Group, init func(*Group), assign func(*Group, *Message)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(message.FieldGid)
	}
	query.Where(predicate.Message(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.MessageColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "gid" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (gq *GroupQuery) loadTask(ctx context.Context, query *TaskQuery, nodes []*Group, init func(*Group), assign func(*Group, *Task)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(task.FieldGid)
	}
	query.Where(predicate.Task(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.TaskColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (gq *GroupQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := gq.ctx.Fields; len(fields) > 0 {
		...
		if gq.withBeacon != nil {
			_spec.Node.AddColumnOnce(group.FieldBid)
		}
		if gq.withOperator != nil {
			_spec.Node.AddColumnOnce(group.FieldAuthor)
		}
	}
