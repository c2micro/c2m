...
func (bq *BeaconQuery) QueryListener() *ListenerQuery {
	query := (&ListenerClient{config: bq.config}).Query()
...
func (bq *BeaconQuery) Clone() *BeaconQuery {
	...
	return &BeaconQuery{
		...
		withListener: bq.withListener.Clone(),
		withGroup:    bq.withGroup.Clone(),
		withTask:     bq.withTask.Clone(),
...
func (bq *BeaconQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Beacon, error) {
	...
	if query := bq.withListener; query != nil {
		if err := bq.loadListener(ctx, query, nodes, nil,
			func(n *Beacon, e *Listener) { n.Edges.Listener = e }); err != nil {
			return nil, err
		}
	}
	if query := bq.withGroup; query != nil {
		if err := bq.loadGroup(ctx, query, nodes,
			func(n *Beacon) { n.Edges.Group = []*Group{} },
			func(n *Beacon, e *Group) { n.Edges.Group = append(n.Edges.Group, e) }); err != nil {
			return nil, err
		}
	}
	if query := bq.withTask; query != nil {
		if err := bq.loadTask(ctx, query, nodes,
			func(n *Beacon) { n.Edges.Task = []*Task{} },
			func(n *Beacon, e *Task) { n.Edges.Task = append(n.Edges.Task, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (bq *BeaconQuery) loadListener(ctx context.Context, query *ListenerQuery, nodes []*Beacon, init func(*Beacon), assign func(*Beacon, *Listener)) error {
...
func (bq *BeaconQuery) loadGroup(ctx context.Context, query *GroupQuery, nodes []*Beacon, init func(*Beacon), assign func(*Beacon, *Group)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(group.FieldBid)
	}
	query.Where(predicate.Group(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(beacon.GroupColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "bid" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (bq *BeaconQuery) loadTask(ctx context.Context, query *TaskQuery, nodes []*Beacon, init func(*Beacon), assign func(*Beacon, *Task)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Beacon)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(task.FieldBid)
	}
	query.Where(predicate.Task(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(beacon.TaskColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (bq *BeaconQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := bq.ctx.Fields; len(fields) > 0 {
		...
		if bq.withListener != nil {
			_spec.Node.AddColumnOnce(beacon.FieldListenerID)
		}
	}
