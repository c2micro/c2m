...
func (tq *TaskQuery) loadGroup(ctx context.Context, query *GroupQuery, nodes []*Task, init func(*Task), assign func(*Task, *Group)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "gid" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TaskQuery) loadBlobberArgs(ctx context.Context, query *BlobberQuery, nodes []*Task, init func(*Task), assign func(*Task, *Blobber)) error {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "args" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TaskQuery) loadBlobberOutput(ctx context.Context, query *BlobberQuery, nodes []*Task, init func(*Task), assign func(*Task, *Blobber)) error {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "output" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TaskQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withGroup != nil {
			_spec.Node.AddColumnOnce(task.FieldGid)
		}
		if tq.withBeacon != nil {
			_spec.Node.AddColumnOnce(task.FieldBid)
		}
		if tq.withBlobberArgs != nil {
			_spec.Node.AddColumnOnce(task.FieldArgs)
		}
		if tq.withBlobberOutput != nil {
			_spec.Node.AddColumnOnce(task.FieldOutput)
		}
	}
