...
func (oq *OperatorQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Operator, error) {
	...
	if query := oq.withGroup; query != nil {
		if err := oq.loadGroup(ctx, query, nodes,
			func(n *Operator) { n.Edges.Group = []*Group{} },
			func(n *Operator, e *Group) { n.Edges.Group = append(n.Edges.Group, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OperatorQuery) loadChat(ctx context.Context, query *ChatQuery, nodes []*Operator, init func(*Operator), assign func(*Operator, *Chat)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(chat.FieldFrom)
	}
...
func (oq *OperatorQuery) loadGroup(ctx context.Context, query *GroupQuery, nodes []*Operator, init func(*Operator), assign func(*Operator, *Group)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Operator)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(group.FieldAuthor)
	}
	query.Where(predicate.Group(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(operator.GroupColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
